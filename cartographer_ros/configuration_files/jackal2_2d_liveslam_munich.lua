...
-- TRAJECTORY_BUILDER_2D.pose_extrapolator.constant_velocity.imu_gravity_time_constant = 0.5
-- TRAJECTORY_BUILDER_2D.pose_extrapolator.constant_velocity.pose_queue_duration = 0.1
-- TRAJECTORY_BUILDER_2D.pose_extrapolator.imu_based.solver_options.num_threads = 6
-- -- calculated from measurements from rosbag in Munich (imu_based extrapolator unused in 2D).
-- -- TRAJECTORY_BUILDER_2D.pose_extrapolator.imu_based.gravity_constant = 9.927842459878196
-- -- MUNICH WEIGHTS START (imu_based extrapolator unused in 2D)
...
-- TRAJECTORY_BUILDER_2D.pose_extrapolator.imu_based.imu_acceleration_weight = 0.
-- -- MUNICH WEIGHTS END
...
options = {
  ...
  num_point_clouds = 1,
  lookup_transform_timeout_sec = 0.2,
  -- Reduced publish rates to lower Cartographer + RViz CPU during live mapping/recording.
  submap_publish_period_sec = 1.0,
  pose_publish_period_sec = 0.02,
  trajectory_publish_period_sec = 1.0,
  rangefinder_sampling_ratio = 1.,
  odometry_sampling_ratio = 1.,
  fixed_frame_pose_sampling_ratio = 1.,
  imu_sampling_ratio = 1.,
