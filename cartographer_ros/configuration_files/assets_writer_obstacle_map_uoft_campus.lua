...
-- Campus-scale fix 2026-09-16: voxel_filter_and_remove_moving_objects removed.
-- Its HybridGrid is anchored at the map origin and capped at 8 doubling levels,
-- so at a 0.05 m voxel it only reaches ~200 m and dies with
-- "Check failed: new_bits <= 8" on any trajectory far from the origin (campus
-- coordinates run to ~550 m). /filtered_point_cloud2 is already the masker's
-- people-removed cloud, so the step was redundant here anyway.
options = {
